...
## Fetch records
	$stmt = $conn->prepare("SELECT inst, sum(tp) as TP,SUM(icp) as ICP,(SUM(icp)/sum(tp)) as ICP_PERCENTAGE FROM year_wise WHERE  1 ".$searchQuery." AND year>=".$min." and year<=".$max." and tp>0 and tc>0 GROUP by inst  ORDER BY ".$columnName." ".$columnSortOrder."  LIMIT :limit,:offset");
///
// $stmt = $conn->prepare("SELECT * FROM employee WHERE 1 ".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT :limit,:offset");Bind values
	...
	foreach($empRecords as $row){
		$data[] = array(
			...
			"tp"=>$row['TP'],
			"icp"=>$row['ICP'],
			"icp_percent"=>$row['ICP_PERCENTAGE']
		);
	}
